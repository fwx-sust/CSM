...
%different hh
n_el_0=[20,30,40,50,60,70,80,90]; %changable
flag=0;
for n_el_xy = n_el_0
    flag=flag+1;
    n_el_x = n_el_xy;% number of elements in x-dir
    n_el_y = n_el_xy;               % number of elements in y-dir
    ...
    for ee = 1 : n_el
        ...
        for ll = 1 : n_int
            ...
            for aa = 1 : n_en
                Na = Quad(aa, xi(ll), eta(ll)); %shape function
                [Na_xi, Na_eta] = Quad_grad(aa, xi(ll), eta(ll));
    ...
    fenzi0=0;
    %actually, fenmu is not necessary.
    % fenmu0=0;
    fenzi1=0;
    % fenmu1=0;
    for ee=1:n_el
        ...
        for qua=1:n_int
            ...
            for aa = 1 : n_en
                uh=uh+u_ele(aa)*Quad(aa, xi(qua), eta(qua)); %qN
            ...
            fenzi0= fenzi0+detJ*weight(qua)*(uh-u)^2;
            % fenmu0=fenmu0+detJ*weight(qua)* (u^2);

            fenzi1= fenzi1+ detJ*weight(qua)*( (uh-u)^2 + (uhx -ux)^2 + (uhy -uy)^2 );
            % fenmu1=fenmu1+detJ*weight(qua)* ( (ux^2)+(uy^2) );
        end
    end
    fenzi0=sqrt(fenzi0);
    % fenmu0=sqrt(fenmu0);
    error_L2(flag)=fenzi0;
    fenzi1=sqrt(fenzi1);
    % fenmu1=sqrt(fenmu1);
    error_H1(flag)=fenzi1;
end
%plot
figure()
plot(log(hh),log(error_L2),'ro');
hold on
A(1,:)=polyfit(log(hh),log(error_L2),1);
xa=linspace(-5,-2,20);ya=A(1,1)*xa+A(1,2);
plot(xa,ya,'-');

plot(log(hh),log(error_H1),'x');
hold on
B(1,:)=polyfit(log(hh),log(error_H1),1);
